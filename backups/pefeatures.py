...
class SectionInfo(FeatureType):
    ...
    def __call__(self, binary):
        ...
        general = [len(binary.sections),  # total number of sections
                   ...
                   _sum(binary.sections, lambda x: x.size == 0),
                   # number of sections with an empty name
                   # sum(1 for s in binary.sections if s.name == ""),
                   _sum(binary.sections, lambda x: x.name == ""),
                   ...
                   _sum(binary.sections, lambda s: s.has_characteristic(lief.PE.SECTION_CHARACTERISTICS.MEM_READ)
                                                   and s.has_characteristic(
                       lief.PE.SECTION_CHARACTERISTICS.MEM_EXECUTE)),
                   # sum(1 for s in binary.sections if s.has_characteristic(
                   #     lief.PE.SECTION_CHARACTERISTICS.MEM_WRITE)),  # number of W
                   _sum(binary.sections, lambda s: s.has_characteristic(
                       lief.PE.SECTION_CHARACTERISTICS.MEM_WRITE))
                   ]
        # TODO: print('genaral part ', general)
        # gross characteristics of each section
        # section_sizes = [(s.name, len(s.content)) for s in binary.sections]
        section_sizes = _convert(binary.sections, lambda s: (s.name, len(s.content)))
        # section_entropy = [(s.name, s.entropy) for s in binary.sections]
        section_entropy = _convert(binary.sections, lambda s: (s.name, s.entropy))
        # section_vsize = [(s.name, s.virtual_size) for s in binary.sections]
        section_vsize = _convert(binary.sections, lambda s: (s.name, s.virtual_size))
...
class ImportsInfo(FeatureType):
    ...
    def __call__(self, binary):
        ...
        libraries = _convert(binary.libraries, lambda l: l.lower())
        ...
        if binary is not None:
            for fe in self.parsed_features:
                try:
                    ...
                    print("featurevectors append error")
...
if __name__ == '__main__':
    bytez = interface.fetch_file("Backdoor.Win32.711")
    extractor = PEFeatureExtractor()
    featurevectors = extractor.extract(bytez)
